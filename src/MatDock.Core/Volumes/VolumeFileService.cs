...
public sealed record FileReadResult(string Content, bool Truncated, bool IsBinary);
...
public sealed class VolumeFileService
{
    ...
    public async Task<(FileReadResult? File, string? Error)> ReadAsync(DockerEnvironment env, string volume, string relPath, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            using (client)
            {
                var result = await RunAsync(client, VolumeFileCommands.Read(head, _options.HelperImage, volume, rel), ct);
                if (result.ExitStatus != 0)
                {
                    return (null, "Datei konnte nicht gelesen werden (existiert sie und ist es eine Datei?).");
                }

                var content = result.StdOut;
                var truncated = content.Length > VolumeFileCommands.MaxReadBytes;
                if (truncated)
                {
                    content = content[..VolumeFileCommands.MaxReadBytes];
                }

                var isBinary = content.Contains('\0');
                return (new FileReadResult(content, truncated, isBinary), null);
            }
        }
        catch (Exception ex)
        {
